...
def student_nameSort(alist, start, end):
    if end-start > 1:
        mid = (start+end)//2
        student_nameSort(alist,start,mid)
        student_nameSort(alist, mid, end)
...
alist = [input(f"Enter {_} Student Name: ") for _ in range(1, userInput+1)]
student_nameSort(alist, 0, len(alist))
